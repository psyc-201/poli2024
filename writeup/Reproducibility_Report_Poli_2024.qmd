...
1.  [x] **Fit the hierarchical Bayesian model (HBM)**
    ...
    -   Fit the model using ADVI sampling.
...
2.  Load the posterior medians from `posterior_median_rep.csv` as the "ground truth" parameters.

3.  Use the medians and the info-theoretic measures to generate a simulated dataset.

4.  (If needed) follow the same pre-processing steps as above with the simulated data.

5.  Run the HBM on the simulated data using ADVI.

6.  Compare the recovered parameters to the original.

    **Specifically, I aim to recover posterior distributions for curiosity and achieve a comparable model fit with the simulated data.**

### Differences from original study

The computing environments are the same. Visualizations may be carried out in R, but the model and any pre-processing steps will be in Python. ADVI will be used instead of MCMC.
...
2.  **Changed from true sampling to optimization for feasibility**:
    ...
    4.  **Assessed ADVI convergence**. The lines flattened out by iteration 10k, indicating that the model successfully reached an "optimal" approximation of the posteriors:
        ...
        ![Convergence plot after fitting model over full dataset.](images/advi_convergence-01.png)
3.  **Fit the model w/ ADVI sampling**:

-   I did this several times with data from just 3 subjects for testing purposes, and then with all subjects in the combined dataset (shown above).
...
As of 11/12/25, I have completed the steps in the Reproduction section of my Methods, and am now working on the simulation script.
